...
//antes de executar la query, algunas validation del form
	...
	*/
	...
	if(!isset($_SESSION['user']))
	{
		...
		$query = "SELECT COUNT(*) FROM Usuario WHERE mail='$mail' AND pass='$pass'";
		...
		if(($result = $link->query($query)))
		{
			if($result->num_rows == 1)//al menos un usuario
			{
				...
				{
					header("location: http://sistemaswebsimonebrazzo.hol.es/Lab4Seguridad/Login.php?err=useropass");
				}	
			}
			else
			    header("location: http://sistemaswebsimonebrazzo.hol.es/Lab4Seguridad/Login.php");	
